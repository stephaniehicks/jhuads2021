...
Often, the human ear can detect the difference between 
...
we can adapt the idea and build a non-linear classifier. 
...
and how it relates to SVMs. First, let's use a plain ole 
hyperplane. 
...
For example, in two dimensions, a hyperplane is
...
the dashed lines are the **support vectors** (they 
...
The purple and blue grid indicates the 
...
**Pro tip**: Although the maximal margin classifier is often successful,
it can also lead to overfitting when $p$ is large. So if you use
this, it's important to explore the use of cross-validation. 
...
**margin** of our hyperplane. 

This works great if **a separating hyperplane exists**. However, 
many times, that isn't true and there is no solution with 
$M > 0$. So instead we can try to find a hyperplane that 
**almost** separates the classes (with some penalty or cost 
associated with each observation that is is on the wrong side 
of the hyperplane).
...
We could consider building a **support vector classifer** 
or a **soft margin classifer** that misclassifies a few 
training observations in order to do a better job of 
classifying the remaining observations. 

The margin is **soft** because it can be violated by some 
...
(also known as **support vectors**) will affect the 
hyperplane (and hence classification). 

This make sense. But why? 

When $C$ is large, the margin is wide, 
...
between the predictors and outcome, a solution is to enlarge the feature 
...
In the next section, we will learn about the **support vector machine**
...
is a **linear kernel** (linear in the features) and would
return the support vector classifier. 

In contrast, 
this kernel is called a **polynomial kernel** of degree $d$. 
If $d > 1$, then the support vector classifier results 
in a more flexible boundary. 
...
to a long format using the `pivot_longer()` function in the 
`tidyr` package. 
